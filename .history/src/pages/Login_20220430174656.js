...
const Login = () => {
  ...
  const emailValidation = () => {
    ...
    if (regEx.test(email)) {
      setIsErrorEmail("Email is Valid");
    } else if (!regEx.test(email) && email !== "") {
      setIsErrorEmail("Email is Not Valid");
    } else {
      setIsErrorEmail("");
    }
  };
              ...
              {({ input, meta }) => (
                <Content>
                  <Grid item xs={12} style={{
                    ...
                    <TextField {...input} onChange={handleOnChange} error={meta.error && meta.touched} type="email" placeholder="Enter email" label="Email" variant="outlined" fullWidth sx={{
                      margin: "10px",
                    }} />
